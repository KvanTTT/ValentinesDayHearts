...
public class ValentinesDayHeartsApp extends ApplicationAdapter implements InputProcessor {
	...
	public void create() {
		...
		bodyLoader = new BodyEditorLoader(Gdx.files.internal("data/hearts.json"));
	...
	private void addHeart(float x, float y) {
		...
		String word = heartWords.size() > 0 ? heartWords.get((int)(Math.random() * heartWords.size())) : "";
		hearts.put(body, new Heart(body, sprite, size, word, font, effect, breakingSound));
	}
